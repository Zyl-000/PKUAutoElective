    ...
    while True:
        ...
        try:
            ...
            if supply_cancel_page == 1:
                ...
                r = page_r = elective.get_SupplyCancel(username=username)
                ...
                while True:
                    ...
                    r = page_r = elective.get_supplement(username=username, page=supply_cancel_page) # 双学位第二页
                    tables = get_tables(r._tree)
                    try:
                        elected = get_courses(tables[1])
                        plans = get_courses_with_detail(tables[0])
                        ...
                        _ = elective.get_SupplyCancel(username=username) # 遇到空页面时请求一次补退选主页，之后就可以不断刷新
                    else:
                        break
                    finally:
                        retry -= 1
